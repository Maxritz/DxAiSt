...
A lazy over-engineering audit found the tree lean: every module is either core, has a production caller, or a dedicated test consumer. No dead modules, no reinvented standard-library routines, and no single-implementation interface hierarchies remain. The previously-shipped `dx_capi` C-ABI wrapper was removed (zero callers), and the `TDRGuard` (GPU device-removal guard) and `Config` (chunk-size / VRAM-margin / async-prefetch knobs) classes were wired into real call sites.
